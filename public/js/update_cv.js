$(function(){
    ...
    if(!localStorage.getItem('edit_cv_done_yet')){
        localStorage.setItem('edit_cv_done_yet', 'no');
    }


    let hasClickedToSubmitForm = false;
     ...
     */
     let numOfEducationItems = phpEducationItems;
     let numOfKeyProgrammingLanguages = phpProgrammingLanguagesItems;
     let numOfUrlLinks = phpUrlLinksItems;
     /**
      ...
      * @param {regExp} regExp How we know which type of dynamic input to remove.
      * @returns void
      */
     function deleteDynamicFormInput(regExp){
        $('.form__container.form__container--less_top_margin').each(function(){
            let thisDeleteInfo = $(this).attr('delete_info');
            if(regExp.test(thisDeleteInfo)){
                $(this).remove();
            }
        });
    }
     ...
     function browserGetFormData(){
        if(localStorage.getItem('edit_cv_done_yet') === 'no'){ return; }
         basicFormItemNames.forEach(function(item){
      ...
      * @param {regExp} regExp The regular expression to identify types of dynamic inputs that should be deleted if the localStorage tells us that none of the given type of dynamic input exists.
      * @returns void
      */
     function displayDynamicFormDataOnLoad(numOfDynamicFormItems, correctNamesArray, addDynamicItemFunctionName, setNewNumOfItemsCallbackFunction, regExp){
         if(!Array.isArray(correctNamesArray)) return;
         if(localStorage.getItem('edit_cv_done_yet') === 'no'){ return; }
         if(numOfDynamicFormItems){
            deleteDynamicFormInput(regExp);
     ...
     //BEGIN OF SECTION THAT SHOULD HAPPEN ON DOCUMENT LOAD
         ...
         displayDynamicFormDataOnLoad(storedNumOfFormEducation, formEducationNames, 'addDynamicEducationItem', setNumOfEducationItems, educationRegex);
         displayDynamicFormDataOnLoad(storedNumOfKeyProgrammingLanguages, keyProgrammingLanguagesNames, 'addDynamicKeyProgrammingLanguage', setNumOfKeyProgrammingLanguages, keyProgrammingLanguageRegex);
         displayDynamicFormDataOnLoad(storedNumOfUrlLinks, urlLinksNames, 'addDynamicUrlLink', setNumOfUrlLinks, urlLinksRegex);
     ...
     function whichTypeOfDynamicInputIsBeingDeleted(deleteInfo){
         if(educationRegex.test(deleteInfo)){
             if(numOfEducationItems > 0){
                 numOfEducationItems--;
             }else if(localStorage.getItem('edit_cv_done_yet') === 'no'){
                setNumOfEducationItems(-1);
             }
